...
app.post('/tasks', function(req, res) {
	...
	task.save(function(err, tasks) {
		if (err) {
			...
			res.json({message: 'Successfully added task', tasks: tasks})
		}
	})
})
...
app.put('/tasks/:id', function(req, res) {
	Task.update({_id: req.params.id}, {$set: {title: req.body.title, description: req.body.description }}, {multi: false}, function(err, results) {
		if (err) {
